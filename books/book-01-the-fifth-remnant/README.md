# Book One: The Fifth Remnant

This directory is the working home for Book One of **The Seventh Kingdom**.

## Workflow

- [`manuscript/`](manuscript/) — active sequential chapter drafts
- [`editor-notes/`](editor-notes/) — developmental, line-edit, and continuity notes
- [`revisions/`](revisions/) — alternate and revision-stage chapter files
- [`final/`](final/) — Founder-approved, publication-ready material
- [`TRANSFER-MANIFEST.md`](TRANSFER-MANIFEST.md) — exact chapter-source availability and unresolved gaps

## Recovered manuscript sources

Exact standalone source text has been recovered and placed for Chapters Four through Seven:

- [`manuscript/chapter-04.md`](manuscript/chapter-04.md) — *The Last Manuscript*
- [`manuscript/chapter-05.md`](manuscript/chapter-05.md) — *The Custodians*
- [`manuscript/chapter-06.md`](manuscript/chapter-06.md) — *The Shepherd*
- [`manuscript/chapter-07.md`](manuscript/chapter-07.md) — *The Road West*

The remaining requested chapters are not represented by invented prose or placeholder files. Their exact recovery state is recorded in the Transfer Manifest.

Nothing in this directory becomes established series canon merely by being drafted. Record approved continuity decisions in [`../../canon/`](../../canon/).
